...
uint32_t  get_flag_64(Elf64_Ehdr *elf, Elf64_Shdr *shdr)
{
    uint32_t ret = 0;
...
void print_flag_64(Elf64_Ehdr *elf, Elf64_Shdr *shdr)
{
    int disp = 0;
    int to_disp = 0;
    ...
    for (int i = 0; i < elf->e_shnum; i++)
        if (shdr[i].sh_type != SHT_SYMTAB || shdr[i].sh_type != SHT_DYNAMIC)
            to_disp = 1;
    disp = (to_disp) ? printf((disp) ? ", HAS_SYMS": "HAS_SYMS") : disp;
...
int check_truncate_elf64(Elf64_Ehdr *elf, Elf64_Shdr *shdr, long unsigned size)
{
